      ...
      if (isSupabaseConfigured && supabase) {
        try {
          ...
          const rows = data.map((p: any) => ({
            ...
            slug: p.slug,
            title: p.title,
            subtitle: p.subtitle || p.heroSubtitle || null,
            category: p.category || (p.pageType === "neighborhood" ? "Neighborhood" : "Landing Page"),
            hero_image: p.hero_image || p.heroImage || null,
            content: p.content || "",
            content_col2: p.content_col2 || p.columnTwoContent || null,
            content_layout: p.content_layout || p.layout || "1-column",
            cta_text: p.cta_text || p.primaryCtaText || null,
            cta_url: p.cta_url || p.primaryCtaUrl || null,
            ...
            treatment_approach: Array.isArray(p.treatment_approach) ? p.treatment_approach : (Array.isArray(p.treatmentApproach) ? p.treatmentApproach : []),
            ...
            seo: {
              ...(p.seo || {}),
              title: p.seoTitle || p.seo?.title || p.title,
              description: p.seoDescription || p.seo?.description || p.subtitle,
