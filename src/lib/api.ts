  ...
  if (isSupabaseConfigured && supabase) {
    try {
      ...
      if (!error && Array.isArray(data) && data.length > 0) {
        return data
          ...
          .map((p: any) => ({
            ...
            slug: p.slug,
            title: p.title,
            subtitle: p.subtitle || p.heroSubtitle || undefined,
            category: p.category || (p.pageType === "neighborhood" ? "Neighborhood" : "Landing Page"),
            pageType: p.pageType || (p.category === "Neighborhood" ? "neighborhood" : "custom"),
            neighborhoodName: p.neighborhoodName || p.neighborhood_name || undefined,
            heroImage: p.hero_image || p.heroImage || undefined,
            heroImageAlt: p.hero_image_alt || p.heroImageAlt || undefined,
            hero_image: p.hero_image || p.heroImage || undefined,
            hero_image_alt: p.hero_image_alt || p.heroImageAlt || undefined,
            heroBadge: p.heroBadge || p.hero_badge || undefined,
            heroTitle: p.heroTitle || p.hero_title || p.title,
            heroSubtitle: p.heroSubtitle || p.hero_subtitle || p.subtitle || undefined,
            content: p.content || "",
            content_col2: p.content_col2 || p.columnTwoContent || undefined,
            columnTwoContent: p.columnTwoContent || p.content_col2 || undefined,
            content_layout: p.content_layout || p.layout || "1-column",
            layout: p.layout || p.content_layout || "one-column",
            cta_text: p.cta_text || p.primaryCtaText || undefined,
            cta_url: p.cta_url || p.primaryCtaUrl || undefined,
            primaryCtaText: p.primaryCtaText || p.cta_text || undefined,
            primaryCtaUrl: p.primaryCtaUrl || p.cta_url || undefined,
            secondary_cta_text: p.secondary_cta_text || p.secondaryCtaText || undefined,
            secondary_cta_url: p.secondary_cta_url || p.secondaryCtaUrl || undefined,
            secondaryCtaText: p.secondaryCtaText || p.secondary_cta_text || undefined,
            secondaryCtaUrl: p.secondaryCtaUrl || p.secondary_cta_url || undefined,
            benefits: Array.isArray(p.benefits) ? p.benefits : (p.sections_data?.benefits?.bullets || []),
            symptoms: Array.isArray(p.symptoms) ? p.symptoms : (p.sections_data?.symptoms?.bullets || []),
            treatmentApproach: Array.isArray(p.treatment_approach) ? p.treatment_approach : (Array.isArray(p.treatmentApproach) ? p.treatmentApproach : (p.sections_data?.treatment_approach?.bullets || [])),
            treatment_approach: Array.isArray(p.treatment_approach) ? p.treatment_approach : (Array.isArray(p.treatmentApproach) ? p.treatmentApproach : (p.sections_data?.treatment_approach?.bullets || [])),
            customSections: Array.isArray(p.custom_sections) ? p.custom_sections : (Array.isArray(p.customSections) ? p.customSections : []),
            custom_sections: Array.isArray(p.custom_sections) ? p.custom_sections : (Array.isArray(p.customSections) ? p.customSections : []),
            sectionsData: p.sections_data || p.sectionsData || {},
            sections_data: p.sections_data || p.sectionsData || {},
            hiddenSections: Array.isArray(p.hidden_sections) ? p.hidden_sections : (Array.isArray(p.hiddenSections) ? p.hiddenSections : []),
            hidden_sections: Array.isArray(p.hidden_sections) ? p.hidden_sections : (Array.isArray(p.hiddenSections) ? p.hiddenSections : []),
            sectionOrder: Array.isArray(p.section_order) ? p.section_order : (Array.isArray(p.sectionOrder) ? p.sectionOrder : []),
            section_order: Array.isArray(p.section_order) ? p.section_order : (Array.isArray(p.sectionOrder) ? p.sectionOrder : []),
            faqs: Array.isArray(p.faqs) ? p.faqs : [],
            seo: p.seo || {},
            seoTitle: p.seoTitle || p.seo?.title || p.title,
            seoDescription: p.seoDescription || p.seo?.description || p.subtitle,
            noIndex: p.noIndex ?? p.seo?.noIndex,
            noFollow: p.noFollow ?? p.seo?.noFollow,
            is_published: p.is_published !== false && p.isPublished !== false,
            isPublished: p.isPublished !== false && p.is_published !== false,
      ...
      if (Array.isArray(pages) && pages.length > 0) {
        return pages
          .filter((p: any) => p.is_published !== false && p.isPublished !== false && !deletedSlugs.has(p.slug))
          .map((p: any) => ({
            ...p,
            pageType: p.pageType || (p.category === "Neighborhood" ? "neighborhood" : "custom"),
            neighborhoodName: p.neighborhoodName || p.neighborhood_name || undefined,
            ...
            layout: p.layout || (p.content_layout === "2-column" ? "two-column" : "one-column"),
            primaryCtaText: p.primaryCtaText || p.cta_text || undefined,
            primaryCtaUrl: p.primaryCtaUrl || p.cta_url || undefined,
            secondaryCtaText: p.secondaryCtaText || p.secondary_cta_text || undefined,
            secondaryCtaUrl: p.secondaryCtaUrl || p.secondary_cta_url || undefined,
